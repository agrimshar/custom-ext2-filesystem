...
void write_superblock(int fd) {
	...
	superblock.s_blocks_per_group  = BLOCK_SIZE * 8; /* 8192 */
	superblock.s_frags_per_group   = BLOCK_SIZE * 8; /* 8192 */
...
void write_inode_table(int fd) {
	...
	const char hello_world_path[] = "hello-world";
	memcpy(hello_inode.i_block, hello_world_path, strlen(hello_world_path) + 1);
